...
func CreateSchemaSupport(options *Options) error {
	...
	has := 0
	if err := db.QueryRow(ctx, `select count(1) as has from pg_namespace where nspname = $1 limit 1;`, "schemasupport").Scan(&has); err != nil {
		return err
	}
	...
	has = 0
	if err := db.QueryRow(ctx, `select count(1) as has from pg_tables where schemaname = $1 and tablename = $2 limit 1;`, "schemasupport", "files").Scan(&has); err != nil {
		return err
	}
...
func LoadExisting(options *Options) (List, error) {
	...
	rows, err := db.Query(ctx, `select path, md5 from schemasupport.files;`)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func Execute(options *Options, run List, stats *Stats) error {
	...
	for _, f := range run {
		...
		if !options.Dry {
			if _, err := tx.Exec(ctx, sql, f.Path, f.MD5); err != nil {
				return err
			}
		}
...
func schema_run_sql(options *Options, tx pgx.Tx, file string) error {
	ctx := options.Ctx
...
func schema_run_csv(options *Options, tx pgx.Tx, file string) error {
	ctx := options.Ctx
